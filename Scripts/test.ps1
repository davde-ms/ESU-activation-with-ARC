...
foreach ($row in $data) {
    ...
    Write-Host ""
    Write-Host "Initial row cores " $row.cores
    ...
    switch ($row.isVirtual) {
        "Virtual" {
            ...
            $coreType = "vCore"
            Write-Host "Sending " $row.cores "Vcores for $coreType for the license $licenseName for processing."
        ...
        "Physical" {
            ...
            $coreType = "pCore"
            Write-Host "Sending " $row.cores "Pcores for $coreType for the license $licenseName for processing."
            CreateESULicense -subscriptionId $subscriptionId -tenantId $tenantId -appID $appID -clientSecret $clientSecret -location $location -licenseResourceGroupName $licenseResourceGroupName -licenseName $LicenseName  -state $state -edition $edition -CoreType $coreType -CoreCount $row.cores
            ; break
        } 
        Default {
